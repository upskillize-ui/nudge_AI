...
PATCH NOTES (v2.1):
- update_status now raises HTTPException(404) instead of returning 200 with error
- analytics endpoint validates `period` (was: ValueError on bad input -> 500)
- manual_nudge no longer hardcodes severity="info"; uses request body
- New /admin/aggregate-features cron-trigger endpoint for ML feature backfill
- Added LectureAttendanceEvent.lecture_id forwarding (idempotency)
"""
import logging
import re
...
from app.models import (Nudge, NudgeEvent, AttendanceTracker, RecordingTracker,
                        AssignmentTracker, StudentFeatures)
...
def wh_attendance(e: LectureAttendanceEvent, _=Depends(verify_webhook), db: Session = Depends(get_db)):
    try:
        n = NudgeEngine(db).process_attendance(
            e.user_id, e.course_id, e.batch_id, e.attended,
            e.lecture_title, e.mentor_id, e.student_name,
            lecture_id=e.lecture_id,  # idempotency key
        )
        return {"ok": True, "nudge_id": n.id if n else None,
                "deduped": n is None and bool(e.lecture_id)}
...
def wh_recording_upload(e: RecordingUploadEvent, _=Depends(verify_webhook), db: Session = Depends(get_db)):
    try:
        NudgeEngine(db).register_recording(
            e.lecture_id, e.course_id, e.batch_id, e.lecture_title,
...
def wh_asgn_upload(e: AssignmentUploadEvent, _=Depends(verify_webhook), db: Session = Depends(get_db)):
    try:
        NudgeEngine(db).register_assignment(
            e.assignment_id, e.course_id, e.title, e.deadline,
...
def wh_quiz(e: QuizScoreEvent, _=Depends(verify_webhook), db: Session = Depends(get_db)):
    try:
        n = NudgeEngine(db).process_quiz(
            e.user_id, e.course_id, e.topic_name, e.score,
...
def wh_login(e: LoginEvent, _=Depends(verify_webhook), db: Session = Depends(get_db)):
    try:
        f = db.query(StudentFeatures).filter(
            StudentFeatures.user_id == e.user_id,
        ...
        if not f:
            ...
            db.add(f)
        f.login_frequency = (f.login_frequency or 0) + (1.0 / 7.0)  # was 0.14 magic number
        # EWMA with alpha=0.5 — documented, not a bug
...
@nf.get("/feed")
def get_feed(user_id: str = Query(...), role: str = Query("student"),
             status: str = Query("unread"), limit: int = Query(20, le=100),
             _=Depends(verify_api), db: Session = Depends(get_db)):
    q = db.query(Nudge).filter(Nudge.user_id == user_id, Nudge.user_role == role)
    if status == "unread":
        q = q.filter(Nudge.status.in_(["pending", "delivered"]))
    q = q.filter((Nudge.expires_at > datetime.utcnow()) | (Nudge.expires_at.is_(None)))
    unread = db.query(func.count(Nudge.id)).filter(
        Nudge.user_id == user_id, Nudge.user_role == role,
    ...
    db.commit()
    return {"nudges": [{"id": n.id, "type": n.nudge_type, "priority": n.priority,
                        "title": n.title, "body": n.body, "cta_text": n.cta_text,
                        "cta_url": n.cta_url, "severity": n.severity, "status": n.status,
                        "created_at": str(n.created_at), "meta": n.metadata_json}
                       for n in nudges],
            "total_unread": unread}
...
def unread_count(user_id: str = Query(...), role: str = Query("student"),
                 _=Depends(verify_api), db: Session = Depends(get_db)):
    total = db.query(func.count(Nudge.id)).filter(
        Nudge.user_id == user_id, Nudge.user_role == role,
        Nudge.status.in_(["pending", "delivered"])).scalar() or 0
    by_sev = {s: c for s, c in db.query(Nudge.severity, func.count(Nudge.id)).filter(
        Nudge.user_id == user_id,
        Nudge.status.in_(["pending", "delivered"])).group_by(Nudge.severity).all()}
    return {"total": total, "by_severity": by_sev}


@nf.patch("/{nudge_id}/status")
def update_status(nudge_id: str, u: StatusUpdate,
                  _=Depends(verify_api), db: Session = Depends(get_db)):
    valid = {"read", "clicked", "dismissed"}
    if u.status not in valid:
    ...
    if u.status == "read":
        n.read_at = now
    elif u.status == "clicked":
        n.clicked_at = now
        # A click implies a read too
        if not n.read_at:
            n.read_at = now
    elif u.status == "dismissed":
        n.dismissed_at = now
...
def batch_read(user_id: str = Query(...), _=Depends(verify_api), db: Session = Depends(get_db)):
    cnt = db.query(Nudge).filter(
        Nudge.user_id == user_id,
        Nudge.status.in_(["pending", "delivered"])
...
def assignments(user_id: str = Query(...), _=Depends(verify_api), db: Session = Depends(get_db)):
    now = datetime.utcnow()
    trackers = db.query(AssignmentTracker).filter(
        AssignmentTracker.user_id == user_id).order_by(AssignmentTracker.deadline).all()
    return {"assignments": [{"id": t.assignment_id, "title": t.title, "type": t.assignment_type,
                             ...
                             "urgency": "critical" if (t.deadline - now).total_seconds() / 3600 < 24
                             and (t.submission_status or "not_started") != "submitted" else "normal"
                             } for t in trackers]}
...
_PERIOD_RE = re.compile(r"^(\d{1,4})d$")
...
def analytics(period: str = Query("30d"), _=Depends(verify_api), db: Session = Depends(get_db)):
    ...
    if not m:
        raise HTTPException(400, "period must look like '7d', '30d', '90d'")
    days = min(int(m.group(1)), 365)
    since = datetime.utcnow() - timedelta(days=days)
    total = db.query(func.count(Nudge.id)).filter(Nudge.created_at >= since).scalar() or 0
    delivered = db.query(func.count(Nudge.id)).filter(
        Nudge.created_at >= since, Nudge.delivered_at.isnot(None)).scalar() or 0
    read = db.query(func.count(Nudge.id)).filter(
        Nudge.created_at >= since, Nudge.read_at.isnot(None)).scalar() or 0
    clicked = db.query(func.count(Nudge.id)).filter(
        Nudge.created_at >= since, Nudge.clicked_at.isnot(None)).scalar() or 0
    by_type = {t: c for t, c in db.query(Nudge.nudge_type, func.count(Nudge.id)).filter(
        Nudge.created_at >= since).group_by(Nudge.nudge_type).all()}
    ts = max(total, 1)
    ds = max(delivered, 1)
    return {"total": total, "delivery_rate": round(delivered / ts, 3),
            "open_rate": round(read / ds, 3),
...
def manual_nudge(req: ManualNudgeRequest, _=Depends(verify_api), db: Session = Depends(get_db)):
    eng = NudgeEngine(db)
    severity = getattr(req, "severity", None) or "info"
    created = []
    for u in req.user_ids:
        n = eng._create(u, "student", req.nudge_type, req.title, req.body,
                        severity, req.priority, req.cta_text, req.cta_url)
        if n:
            created.append(n.id)
...
def overview(_=Depends(verify_api), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "at_risk_students": db.query(func.count(AttendanceTracker.id)).filter(
            AttendanceTracker.consecutive_misses >= 3).scalar() or 0,
        ...
        "unwatched_recordings": db.query(func.count(RecordingTracker.id)).filter(
            ...
            RecordingTracker.expected_by < now).scalar() or 0,
        "total_students_tracked": db.query(func.count(func.distinct(
            AttendanceTracker.user_id))).scalar() or 0,
...
def train_dropout(_=Depends(verify_api), db: Session = Depends(get_db)):
    ...
    features = db.query(StudentFeatures).filter(
        StudentFeatures.dropped_out.isnot(None)).all()
    if len(features) < settings.min_training_records:
        raise HTTPException(
            400,
            f"Need {settings.min_training_records}+ labeled records, have {len(features)}")
    df = pd.DataFrame([{
...
@ma.post("/admin/aggregate-features")
def aggregate_features(_=Depends(verify_api), db: Session = Depends(get_db)):
    """Manually trigger the daily feature aggregator. Useful for backfill."""
    updated = NudgeEngine(db).aggregate_features_daily()
    return {"ok": True, "rows_updated": updated}


@ma.get("/admin/recent-nudges")
def recent_nudges(limit: int = Query(20, le=100),
                  _=Depends(verify_api), db: Session = Depends(get_db)):
    """Admin endpoint: fetch latest nudges across ALL students."""
